...
type TWin = {LEFT: number, WIDTH: number};
...
class Math2D {
    ...
    getZeros(f: TFn, { LEFT, WIDTH }: TWin): (number | null)[] {
        let x = LEFT;
        const dx = WIDTH / 100;
        const zeros = [];
        while (x < WIDTH + LEFT) {
    ...
    getNotIntegral(f1: TFn, f2: TFn, a: number, b: number, dx = 0.001) {
        let x = a;
        let s = 0;
        while (x <= b) {
    ...
    pointsVertical(f: TFn, { LEFT, WIDTH }: TWin, dx = 0.001) {
        const points = [];
