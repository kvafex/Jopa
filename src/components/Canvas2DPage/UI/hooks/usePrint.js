const usePrint = (WIN, math) => {
    let canvas = null;
    let mouseX = null;

    const setCanvas = (c) => {canvas = c};

    const setMouseX = (m) => {mouseX = m};

    const printVerticalLine = (x, color = 'rgba(78, 63, 63, 0.67)') => {
        const { BOTTOM, HEIGHT } = WIN;
        const TOP = HEIGHT + BOTTOM;
        canvas.line(x, BOTTOM, x, TOP, color, 1, true);
    }

    const printFunction = (f, color = '#f00', width = 2) => {
        const { LEFT, WIDTH } = WIN;
        let x = LEFT;
        const dx = WIDTH / 100;
        const points = math.pointsVertical(f, { LEFT, WIDTH });
        while (x < WIDTH + LEFT) {
    ...
    const printParametric = ({ x, y, range, color, width }) => {
        let t = range[0];
    ...
    const printNotIntegral = (f1, f2, a, b, color = 'rgba(33, 0, 221, 0.44)') => {
        const points = [];
        let x = a;
        const dx = (b - a) / 100;
        while (x <= b) {
