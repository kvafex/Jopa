...
const FuncUI: React.FC <{
    ...
    const getNotIntegral = (): void => {
        if (funcsI.length >= 2) {
            (NIRef.current as HTMLInputElement).disabled = true;
        } else {
            (NIRef.current as HTMLInputElement).disabled = false;
        }
    ...
    <div className="mar_b">
            <div className="flex_r">
                ...
                <div className="flex_c mar_l">
                    ...
                    <label>Взять эту функцию для интеграла из двух функции?<input ref={NIRef} onChange={() => getNotIntegral()} type="checkbox" className="checkbox"/></label>               
                </div>
            </div>
    </div>
    );
