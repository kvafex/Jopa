...
import { Figure, BottleKleine } from "../../../../modules/math3D";

type TSettings = {
    figure: Figure
}

const BottleKleineSettings: React.FC <TSettings> = ({figure}) => {
    ...
    const keyupInputX = (event: React.KeyboardEvent) => {
        ...
        if (!isNaN(value)) {
            (figure as BottleKleine).setCenter(value, y, z);
        }
    }
    ...
    const keyupInputY = (event: React.KeyboardEvent) => {
        ...
        const z = Number(zRef.current.value);
        if (!isNaN(value)) {
            (figure as BottleKleine).setCenter(x, value, z);
        }
    }
    ...
    const keyupInputZ = (event: React.KeyboardEvent) => {
        ...
        const y = Number(yRef.current.value);
        if (!isNaN(value)) {
            (figure as BottleKleine).setCenter(x, y, value);
        }
    }
    ...
    const keyupInputCount = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 4) {
            (figure as BottleKleine).setCount(value);
        }
    }
    ...
    const keyupInputColor = (event: React.KeyboardEvent) => {
        ...
        if (value.length === 7) {
            (figure as BottleKleine).setColor(value);
        }
    }
...
export default BottleKleineSettings;