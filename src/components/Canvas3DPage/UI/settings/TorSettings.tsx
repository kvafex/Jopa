...
import { Figure, Tor } from "../../../../modules/math3D";

type TSettings = {
    figure: Figure
}

const TorSettings: React.FC <TSettings> = ({figure}) => {
    ...
    const keyupInputX = (event: React.KeyboardEvent) => {
        ...
        if (!isNaN(value)) {
            (figure as Tor).setCenter(value, y, z);
        }
    }
    ...
    const keyupInputY = (event: React.KeyboardEvent) => {
        ...
        const z = Number(zRef.current.value);
        if (!isNaN(value)) {
            (figure as Tor).setCenter(x, value, z);
        }
    }
    ...
    const keyupInputZ = (event: React.KeyboardEvent) => {
        ...
        const y = Number(yRef.current.value);
        if (!isNaN(value)) {
            (figure as Tor).setCenter(x, y, value);
        }
    }
    ...
    const keyupInputRadius = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 0) {
            (figure as Tor).setRadius(value);
        }
    }

    const keyupInputBigRadius = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 0) {
            (figure as Tor).setBigRadius(value);
        }
    }
    ...
    return (<div className="flex_r mar_b">
            ...
            <input className="cInput output mar_r" placeholder="R" onKeyUp={(event) => keyupInputBigRadius(event)}></input>
