...
import { Figure, Cone } from "../../../../modules/math3D";

type TSettings = {
    figure: Figure
}

const ConeSettings: React.FC <TSettings> = ({figure}) => {
    ...
    const keyupInputX = (event: React.KeyboardEvent) => {
        ...
        if (!isNaN(value)) {
            (figure as Cone).setCenter(value, y, z);
        }
    }
    ...
    const keyupInputY = (event: React.KeyboardEvent) => {
        ...
        const z = Number(zRef.current.value);
        if (!isNaN(value)) {
            (figure as Cone).setCenter(x, value, z);
        }
    }
    ...
    const keyupInputZ = (event: React.KeyboardEvent) => {
        ...
        const y = Number(yRef.current.value);
        if (!isNaN(value)) {
            (figure as Cone).setCenter(x, y, value);
        }
    }
