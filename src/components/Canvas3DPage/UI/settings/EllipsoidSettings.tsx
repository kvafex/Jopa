...
import { Figure, Ellipsoid } from "../../../../modules/math3D";

type TSettings = {
    figure: Figure
}

const EllipsoidSettings: React.FC <TSettings> = ({figure}) => {
    ...
    const keyupInputX = (event: React.KeyboardEvent) => {
        ...
        if (!isNaN(value)) {
            (figure as Ellipsoid).setCenter(value, y, z);
        }
    }
    ...
    const keyupInputY = (event: React.KeyboardEvent) => {
        ...
        const z = Number(zRef.current.value);
        if (!isNaN(value)) {
            (figure as Ellipsoid).setCenter(x, value, z);
        }
    }
    ...
    const keyupInputZ = (event: React.KeyboardEvent) => {
        ...
        const y = Number(yRef.current.value);
        if (!isNaN(value)) {
            (figure as Ellipsoid).setCenter(x, y, value);
        }
    }
    ...
    const keyupInputA = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 0) {
            (figure as Ellipsoid).setA(value);
        }
    }
    ...
    const keyupInputB = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 0) {
            (figure as Ellipsoid).setB(value);
        }
    }
    ...
    const keyupInputC = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 0) {
            (figure as Ellipsoid).setC(value);
        }
    }
    ...
    const keyupInputCount = (event: React.KeyboardEvent) => {
        const value = Number((event.target as HTMLInputElement).value);
        if (value > 4) {
            (figure as Ellipsoid).setCount(value);
        }
    }
    ...
    const keyupInputColor = (event: React.KeyboardEvent) => {
        ...
        if (value.length === 7) {
            (figure as Ellipsoid).setColor(value);
        }
    }
